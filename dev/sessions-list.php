	...
	<div class="container">
       ...
       $date = DateTime::createFromFormat("m/d/Y" , $from_date);
		...
		<td>
                 ...
                 $data= mysql_fetch_assoc(mysql_query(" SELECT * FROM `schools` WHERE SchoolId=".$row['school_id']))  ;     
                  $sel=(isset($school_id)&&$school_id==$row['school_id'])?'selected':NULL;
                     $arr_school[$row['school_id']]=$data['SchoolName'];
                  
                  ?>
                     <option 
                         value="<?=$row['school_id']?>" <?=$sel?>  ><?=$data['SchoolName']?></option>
                    <?php } ?>
                                </select>
                    
                    <?php print_r($arr_school);?> <br/>
				...
				<form id="form-manager" class="content_wrap" action="" method="post">
					<div class="ct_heading clear">
						<h3>Manager Tutor Sessions(<?=$tot_record?>)</h3>
						<ul>
