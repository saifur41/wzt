...
class Project extends CI_Model
{
    ...
    public function updateSubbyTime($project_id,$User_id,$inout,$entry_id=0)
    {
         ...
         {
              $query = "update  checkmate_subby_timesheet set project_id = '".$project_id."' , user_id = '".$User_id."' , time_out = now() where id=".$entry_id." AND time_in is not null ";
             // echo $query; die; 
              return $this->db->query($query);
         }
    }
    ...
    //News//
      ...
      public function getSubbyTimeSheet($project_id,$user_id=0,$all_location = 0 ){
            ...
            */
            ...
            $SQL_Query = "  Select intime.id, intime.project_id, intime.user_id, intime.time_in as time ,'IN' as lastactivity_name from checkmate_subby_timesheet intime  UNION 

  Select intime.id, intime.project_id, intime.user_id, intime.time_out as time ,'OUT' as lastactivity_name from checkmate_subby_timesheet intime WHERE project_id =   ".$project_id." ORDER BY time DESC" ;

             if($user_id>0){
 $SQL_Query = "  Select intime.id, intime.project_id, intime.user_id, intime.time_in as time ,'IN' as lastactivity_name from checkmate_subby_timesheet intime  UNION 

  Select intime.id, intime.project_id, intime.user_id, intime.time_out as time ,'OUT' as lastactivity_name from checkmate_subby_timesheet intime WHERE project_id =   ".$project_id." AND user_id = ".$user_id ." ORDER BY time DESC" ;


             }
             

           return $results=  $this->db->query($SQL_Query)->result_array();
            //  $array=array();
                  ...
                  //print_r($proj_sum->projsum); die; 
                   ...
                   // print_r($project_milestones); die;
                    ...
                    if(count($project_milestones) > 0) {
                        ...
                        $response['projects'][$ctr]['project_name'] = $user_projects[$i]->project_name;
                        $response['projects'][$ctr]['project_id'] = $project_id;
                        $response['projects'][$ctr]['project_address'] = $user_projects[$i]->project_address;
                        $response['projects'][$ctr]['project_lat'] = $user_projects[$i]->project_lat;
                        $response['projects'][$ctr]['project_lng'] = $user_projects[$i]->project_lng;
                        $response['projects'][$ctr]['project_site_map'] = $user_projects[$i]->project_site_map;

                        ///////////////
                        //$response['projects'][$ctr]['milestones_sum'] =$proj_mile_sum;
                        $response['projects'][$ctr]['project_total_expenses'] =$project_total_expense;
                        $response['projects'][$ctr]['milestones'] = $project_milestones;
