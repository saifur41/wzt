...
//    print_r($system_warnings); die; 
 ...
 /* this API Update session  value*/

   public function sessionInfoUpdate_post()
  {
      ...
      $res = [];
       ...
       else if ($this->post('drhomework_ses_id') == "")
        {
            $res['status']     =    false;
            $res['response']   =  "Please enter session id.";
            $this->response($res);
        ...
        $Tutoring_client_id         =         $this->post('Tutoring_client_id');
        $drhomework_ses_id          =         $this->post('drhomework_ses_id');
        $data_row                   =         $this->post('data_row');
        $data                       =         array('session_stu_data'=>$data_row);
...
public function SessionDetails_post()
{
    ...
    if ($this->post('Tutoring_client_id') == "")
        {
            $res['status'] = false;
            $res['response']['message'] = "Please enter client_id.";
            $this->response($res);
        }
        ...
        $this->db->select('id,tut_teacher_id,tut_status,drhomework_ref_id as sessionID ,ses_start_time');
        $this->db->from('int_schools_x_sessions_log');
        $this->db->where("Tutoring_client_id='".$Tutoring_client_id."' AND drhomework_ref_id > 0 

         AND ses_start_time between '$start_date.' AND '$end_date' ORDER BY ses_start_time ASC" );

        $assign_list = $this->db->get()->result_array();
        if(isset($assign_list))
          {
              $res['status'] = true;  
              $res['response'] = $assign_list;
              $this->response($res);
          }
        else
        {
              $res['status'] = false;
              $res['response']['message'] = "Something went wrong while adding, please try again later";
              $this->response($res);
        }  
    ...
    public function docInfoUpdate_post()
    {
      $res = [];
      $app_warnings = [];
      if ($this->post('Tutoring_client_id') == "")
      ...
      if ($this->post('drhomework_ses_id') == "")
      {
        ...
        $drhomework_ses_id = $this->post('drhomework_ses_id');
        $assignment_data = $this->post('assignment_data');
        $data=array('doc_after_job_claimed'=>$assignment_data);
