...
. "created_date='$today',teacher_id='1999',district_id='$district_id', ios_newrow = '$ios_newrow',bilingual_test = '".@$bilingual_test."',certificate = '".$certificate."'";
...
{ //child session
  ...
  . "created_date='$today',parent_id='$parentid',district_id='$district_id', ios_newrow = '$ios_newrow' ,bilingual_test = '".@$bilingual_test."',certificate = '".$certificate."'";
}


                    //  echo $sql_session; exit;;
                            // Last lesson QUIZ
...
// Send notifications /// 
  ...
  # $error=(count($_POST['lesson'])).'-Sessions Created!=='.$get_class_id; //die ;
   ...
   if($curr_active_board=='newrow'){
    ...
    //print_r($_SESSION['ses_list_ids']); die; 
    header("Location:create_session_step2.php?id=".$first_ses_id); exit;

   }
   else if($curr_active_board=='wiziq'){

    //print_r($_SESSION['ses_list_ids']); die; 
    header("Location:create_session_success.php?id=".$first_ses_id); exit;
                    ...
                    <div class="ct_display clear">
                        <form name="form_passage" id="form_passage" method="post" action="" enctype="multipart/form-data">
                            <h4 title="Create Session:">Create Session:</h4>
                             ...
                             $Arrvirtual_board=array('newrow'=>'Newrow Virtual Board','1'=>'IOS - Newrow Virtual Board',  'wiziq'=>'wizIQ Virtual Board' );
...
LEFT JOIN terms t ON p.grade_level_id = t.id WHERE p.school_id =".$school_id;
                     ...
                     <label for="lesson_name">Tutor/Instructor Certification Required:</label>
                                   ...
                                   <select name="ses_language" id="ses_language" class="textbox" onchange="javascript:if(this.value == 1) { $('#certificate_area').show(); $('#lesson_submit_id').attr('onclick', 'return confirm(\'Are you sure of these requirements associate with the teacher certificate and bilingual? \')');} else { $('#certificate_area').hide(); $('#lesson_submit_id').removeAttr('onclick'); }">
                                   ...
                                   <div class="radio" >
                                    <label>
                                      <input type="radio" name="certificate" id="optionsRadios1" value="1" >
                                     Bilingual Tutor ( Spanish )
                                    </label>
                                  </div>
                                  <div class="radio">
                                    <label>
                                      <input type="radio" name="certificate" id="optionsRadios2" value="2" checked>
                                      Certified Teacher
                                    </label>
                                  </div>
                                  <div class="radio ">
                                    <label>
                                      <input type="radio" name="certificate" id="optionsRadios3" value="3" >
                                      Certified Teacher in ESL
                                    </label>
                                  </div>
                                   <div class="radio ">
                                    <label>
                                      <input type="radio" name="certificate" id="optionsRadios4" value="4" >
...
<label class="col-md-12" for="date" style="padding-left:0px">Duration:</label>
  <input type="number" min="15" max="60" step="5" required class="form-control" name="duration" value="50">
                              ...
                              <p>
                                    ...
                                    <select name="lesson[]" id="lesson" class="textbox" required="">
           ...
           <button type="submit" name="create_submit"
            id="lesson_submit_id" class="form_button" value=""  onclick="return confirm('Are you sure of these requirements associate with the teacher certificate and bilingual? ')">Create Session</button>
