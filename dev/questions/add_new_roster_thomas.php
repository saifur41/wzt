    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)){
       ...
       $target_file = $target_dir.$School.'_'.$newfilename;
        ...
        $roseterID=mysql_insert_id();
             ...
             if (($handle = fopen($uploads_dir, "r")) !== FALSE) 
              while($line = fgetcsv($handle, 1000, ",")){
                ...
                if($d >= 1) {
                    ...
                    /*check grade is gratjer then 8*/

                      $gradeNO=trim($line[5]);
                      $Grade=  $arrG[$gradeNO];
                       $classname=$line[6];
                      $class_code =0;
                      $teacher_first_name =$line[7];
                      $teacher_last_name =$line[8];
                      $teacher_email = trim($line[9]);
                      $Tuname=$teacher_last_name.'123';
                      $teacher_unique_id =0;
                      $Tpass=md5(123456);

                    $qr=  mysql_query("SELECT id  FROM users WHERE email='".$teacher_email."' && email !=''");
           ...
           $qr=mysql_query("SELECT id  FROM classes WHERE 
            class_name='".$className."' && `school_id`=$school_id  && `grade_level_id`='".$grade_level_id."'  && `teacher_id`=$teacher_id");
                      ...
                      $class_ID= $res['id'];
                         ...
                         echo  $str="INSERT INTO  `classes` SET `class_name`='".$className."',`roster_id`= $roseterID,`class_code`='".$class_code."',`teacher_id`=$teacher_id,`grade_level_id`='".$grade_level_id."',
                      ...
                      if($class_ID>0){
                        ...
                        mysql_query($str);
                          ...
                          $stuname=strtolower(str_replace(" ","",$StuUname));
                              ...
                              $num_rows = mysql_num_rows($qr);
                                ...
                                if($num_rows > 0 ){
                                  ...
                                  $student_id= $res['id'];
                                   ...
                                   echo  $str= "INSERT INTO `students` SET `email`='".$stuEmail."',
                                    `first_name` ='".$stuFName."',  `last_name`  ='".$stuLName."', `student_bdy`='".$stuBdy."',  `username`  = '".$stuname."',  `password`  ='".$StuPass."',   `school_id`  ='".$School."',  `grade_level_id` ='".$Grade."',   `roster_id` ='".$roseterID."', `uid` ='".$stuUID."',  `status` =1, `created` ='".$datetm."'";
                               ...
                               mysql_query($str);
                                    $student_id =mysql_insert_id();
                                     ...
                                     if($student_id>0){
                                      ...
                                      $qr=mysql_query("SELECT id FROM students_x_class WHERE `class_id`=$class_ID && `student_id`=$student_id && `grade_level_id`='".$Grade."'");
                                       $num_rows = mysql_num_rows($qr);
                                       if($num_rows ==0 ){
...
echo '<script>window.location="add_new_roster_thomas.php"</script>';
   }
   ?>
                        ...
                        <tbody>
                           <?php
                              ...
                              <td align="center">
                                ...
                                <a href="add_new_roster_thomas.php?delid=<?= $res['id']?>" style="color: red"onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i></a>
                              </td>
                            </tr>
                           <?php 
                              $i++; } ?>
