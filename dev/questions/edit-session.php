...
$_POST['quiz_id']=(isset($_POST['quiz_id']))?$_POST['quiz_id']:0;
 ...
 $mster= mysql_fetch_assoc(mysql_query(" SELECT * FROM int_schools_x_sessions_log WHERE id=".$_GET['ses']));   
   ...
   $special_notes=NULL;
    ...
    $qwww=" UPDATE int_schools_x_sessions_log SET quiz_id='".$_POST['quiz_id']."', special_notes='".$special_notes."',tut_status='STU_ASSIGNED'  WHERE id='".$_GET['ses']."' ";   
    $gradeid=$_POST['grade'];
    ...
    $del=mysql_query(" DELETE FROM int_slots_x_student_teacher WHERE slot_id=".$_GET['ses']);
    /////////////
     for ($k = 0; $k <$tot_std; $k++) {
                   // $today
         $sid.=$_POST['student'][$k];
    ...
    <div class="container">
      ...
      $at_time=date_format(date_create($ses_start_time), 'h:i a');
                   ...
                   <h4 style="color: #1b64a9; font-size:16px">Session Date/Time:&nbsp;<?=$sdate?> at-<?=$at_time?>
                       <?=($in_sec>-3600)?NULL:",&nbsp;Can't Re-assign students, session expired";?></h4>
    ...
    $("#gradeid").change(function(){
     ...
     // alert('hghgh'+gjd); // alert(this.val()); 
        ...
        function(data,status){
            ...
            //alert("Data: " + data + "\nStatus: " + status);
                        ...
                        ?>
                                  ...
                                  <p>
                                        ...
                                        <select name="grade" id="gradeid" class="required textbox">
                                         ...
                                         $selected = ($folder['id'] ==$sel_grade_id) ? ' selected="selected"' : '';
                   ...
                   <select name="class" id="tclassid" class="required textbox"
                           onchange="open_asses('<?php print $base_url.'edit-session.php?ses='.$_GET['ses'].'&cl='?>',
                                       $(this).val());">
        ...
        . 'WHERE teacher_id = \'' . $user_id . '\' ');  
                                    ...
                                    if($result['grade_level_id']!=$sel_grade_id)continue;    
                                           ////////////////////     
                                 $selected = (isset($_GET['ses'])&&$result['id'] ==$sel_class_id) ? ' selected="selected"' : '';
...
//                                               
                           ...
                           //$district_qry = mysql_query('SELECT * from loc_district ORDER BY district_name ASC ');
                                           ...
                                           $qq=" AND class_id='".$sel_class_id."' ";
                                $res= mysql_query(" SELECT * FROM students WHERE 1 ".$qq);   
                        ...
                        // $res= mysql_query(" SELECT * FROM students WHERE 1 Limit 10");      
                             if(isset($_POST['student']))
                                 $st_arr=$_POST['student'];// save#change
                                    ...
                                    <select class="form-control" name="student[]" id="district" multiple="true">
                                        <?php while ($std= mysql_fetch_assoc($res)) { 
                                          $st_name=$std['first_name']." ".$std['middle_name']." ".$std['last_name'];  
                                            ?>
            <option <?php if (in_array($std['id'],$st_arr)) { ?> selected="selected" <?php } ?> value="<?php print $std['id']; ?>">
                                                <?=$st_name?></option>
...
<!--                      <select name="quiz_id" class="required textbox"   >-->
        ...
        // $res_q= mysql_query(" SELECT * FROM int_quiz WHERE grade_id='".$clss_det['grade_level_id']."' ");   
         $res_q= mysql_query(" SELECT * FROM int_quiz WHERE grade_id='".$sel_grade_id."' ");   
         $j=1;
         ...
         while ($row= mysql_fetch_assoc($res_q)) {
          ...
          $selected = ($row['id']==$quiz_id) ?"selected":NULL;   
             ?>      
             <option  value="<?=$row['id']?>" <?=$selected?>  ><?=$row['objective_name']?></option>   
...
information to help your student, such as distractors to focus on, concepts, etc...</p> 
                                   ...
                                   <textarea class="form-control text-info" style="color: blue;"
                             name="special_notes" placeholder="Info" rows="5"><?=$special_notes?></textarea> 
                   ...
                   <input type="submit" name="add_student" id="" class="form_button submit_button" value="Submit" />
                           ...
                           <?php  }else echo 'Expired,can not Re-assign students,'
