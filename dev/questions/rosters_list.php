...
/////////Update Assesment///////////
 ...
 if(isset($_GET['del_id'])){
    $getid=$_GET['del_id'];
    $roster=mysql_fetch_assoc(mysql_query("SELECT * FROM school_master_rosters WHERE id=".$getid));
  if(unlink('uploads/rosters/'.$roster['filename'])){
 ...
 // $del_roster=mysql_query(" DELETE FROM school_master_rosters WHERE id=".$getid);
   ...
   // echo 'file delter'; die; 
    ...
    $d2=mysql_query(" DELETE FROM classes WHERE roster_id=".$getid);
                ...
                <?php  
                  ...
                  $childs = mysql_query($dqq);       
                    ...
                    //$childs = mysql_query('SELECT * FROM int_quiz ORDER BY created DESC');      
                     ...
                     $sql="SELECT s.SchoolName, r. * FROM school_master_rosters r INNER JOIN schools s ON r.school_id = s.SchoolId WHERE 1 ";
                           ...
                           // http://intervene.srinfosystem.com/questions/uploads/sample-questions.csv
                            ...
                            <?=$row['SchoolName']?> </strong><br/>
                             <?=$row['filename']?> <br/>
                                ...
                                <br/>
                         <a href="uploads/rosters/<?=$row['filename']?>"
                          class="btn btn-primary btn-sm">Download CSV</a>       
                                                           ...
                                                           <td>     
                  <strong class="text-primary"><?=date('F d,Y',strtotime($row['created']))?> </strong> 
                                                         </td>
                                                          <td>
                                                     <?=$row['status']?><br/>
<?php if($row['school_id']==130){?>
                         <a   title="process Roster" href="process_roster_test.php?id=<?=$row['id']?>">
  ...
  ?>
                         ...
                         <a   title="process Roster" href="process_roster.php?id=<?=$row['id']?>">
                                          <span class="glyphicon glyphicon-pencil"></span></a>
  <?php
}

                                           ?>
                                            <a   title="Delete Roster" href="rosters_list.php?del_id=<?=$row['id']?>">
