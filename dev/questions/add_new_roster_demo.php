              ...
              while($line = fgetcsv($handle, 1000, ",")){
                ...
                if($d >= 1) {
                    ...
                    /*check grade is gratjer then 8*/
                      ...
                      $classname=$line[4];
                      $class_code =0;
                      $teacher_first_name =$line[5];
                      $teacher_last_name =$line[6];
                      $teacher_email =$line[7];
                      $Tuname=$teacher_last_name.'123';
                      $teacher_unique_id =$line[8];;
                      $Tpass=md5(123456);
                 ...
                 $teacher_id= $res['id'];
                  ...
                  `date_registered`='".$datetm."', `district_id`=0, `master_school_id`=0, `q_remaining`=0";
                   ...
                   if($teacher_id > 0 ){
                        ...
                        $school_id=$School;
                        $res = mysql_fetch_assoc(mysql_query("SELECT id ,count(id) as cnt FROM classes WHERE 
                          class_name='".$className."' && `school_id`=$school_id  Group by id"));
                        if($res['cnt'] > 0 ){
                         $class_ID= $res['id'];
                        }
                        else{
                            ...
                            if($class_teach_ID>0){
                                ...
                                if($res['cnt'] > 0 ) {
                                  $student_id= $res['id'];
                                    ...
                                    $str= "INSERT INTO `students` SET  `first_name` ='".$stuFName."',  `last_name`  ='".$stuLName."', `student_bdy`='".$stuBdy."',  `username`  = '".$stuname."',  `password`  ='".$StuPass."',   `school_id`  ='".$School."',  `grade_level_id` ='".$Grade."',   `roster_id` ='".$roseterID."', `uid` ='".$stuUID."',  `status` =1, `created` ='".$datetm."'";
...
echo '<script>window.location="add_new_roster_demo.php"</script>';
   }
   ?>
                        ...
                        <tbody>
                           <?php
                              ...
                              <td align="center">
                                ...
                                <a href="add_new_roster_demo.php?delid=<?= $res['id']?>" style="color: red"onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i></a>
                              </td>
                            </tr>
                           <?php 
                              $i++; } ?>
