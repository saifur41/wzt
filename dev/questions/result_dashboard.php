<?php
   ...
   if ($_GET['cid']) {
       ...
       if($class_res['grade_level_id'] > 0) {
          ...
          // SQL : completed assessments in class by students>0

        $str=" SELECT res.assessment_id as res_assessment_id,res.class_id, ax.*,asm.assesment_name,asm.id as assessment_org_id
          FROM students_x_assesments res
          Left Join teacher_x_assesments_x_students ax
          ON res.assessment_id=ax.assessment_id
          Left Join assessments asm
          ON  res.assessment_id=asm.id WHERE   ax.status='Completed' AND res.class_id= '$get_class_id' 
          GROUP BY res.assessment_id ";

          $asses_res=mysql_query($str);
        }
   }
                  ...
                  <form name="form_class" id="form_class" method="get" action="data_dash.php" enctype="multipart/form-data">
                     <div class="add_question_wrap clear fullwidth">
                        ...
                        <p>
                           ...
                           <label for="lesson_name">Choose Assessment</label>
                           <?php while ($assesments = mysql_fetch_assoc($asses_res)) {
                              $assesments['assesment_name']=($assesments['created_by']>0)?'Principal-created-'.$assesments['assesment_name']:$assesments['assesment_name']; 
                               ...
                               ?>
                           <br />
                           <input type="radio" name="assesment" value="<?php print $assesments['assessment_org_id'] ?>" <?php if ($assesments['assessment_org_id'] == $_GET['assesment']) { ?> checked="" <?php } ?> /> <?php print $assesments['assesment_name'] ?>
                           <?php
                              }
