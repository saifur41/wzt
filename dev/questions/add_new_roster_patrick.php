             ...
             if (($handle = fopen($uploads_dir, "r")) !== FALSE) 
              while($line = fgetcsv($handle, 1000, ",")){
                ...
                if($d >= 1) {
                   ...
                   $Grade=$line[3];
                    $gradeNO=str_replace("th",'',$Grade);
                    $gradeNO = trim(preg_replace('/\s+/', '', $gradeNO));
                    $Grade=  $arrG[$gradeNO];

                    $Student=$line[5];
                    $stuArr= explode(',', $Student);

                    $Institute=$line[7];
                    $insArr = explode(',', $Institute);

                    $teacher_first_name =$insArr[1];
                    $teacher_last_name =$insArr[0];
                    $classname=$line[8];
                    $teacher_email =$line[9];
                   ...
                   if($teacher_id > 0 ){
                        ...
                        $qr=mysql_query("SELECT id  FROM classes WHERE  class_name='".$className."' && `school_id`=$school_id  && `grade_level_id`='".$grade_level_id."'  && `teacher_id`=$teacher_id");
                        $num_rows = mysql_num_rows($qr);
                        if($num_rows > 0 ){
                              ...
                              $num_rows = mysql_num_rows($qr);
                                ...
                                if($num_rows > 0 ){
                                  ...
                                  $student_id= $res['id'];
                                   ...
                                   $str= "INSERT INTO `students` SET `email`='".$stuEmail."',
...
echo '<script>window.location="add_new_roster_patrick.php"</script>';
   }
   ?>
                        ...
                        <tbody>
                           <?php
                              ...
                              <td align="center">
                                ...
                                <a href="add_new_roster_patrick.php?delid=<?= $res['id']?>" style="color: red"onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i></a>
                              </td>
                            </tr>
                           <?php 
                              $i++; } ?>
