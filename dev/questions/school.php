...
function _CountStudentTeacher($TeacherId){

$str="SELECT class_id FROM `class_x_teachers` WHERE `teacher_id` = '".$TeacherId."'";
$res =mysql_query($str);
...
$str="SELECT count(student_id) as total FROM `students_x_class` WHERE  `class_id`  IN ($clID)";

$r=mysql_fetch_assoc(mysql_query($str));
            ...
            if (mysql_num_rows($users) > 0) {
                ...
                while ($user = mysql_fetch_assoc($users)) {
                 //print_r($total_students);
                    ...
                    $totalStu= _CountStudentTeacher($tid);
                    $shared_terms = ($user['shared_terms'] != NULL) ? explode(',', $user['shared_terms']) : array();
                    echo "<tr>";
                    echo "<td>$i</td>";
                    echo "<td>{$user['first_name']} {$user['last_name']}</td>";
                    echo "<td>{$user['email']}<br>Number Of Student Per Teacher: {$totalStu}</td>";
                    echo "<td>";
