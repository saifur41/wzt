...
include("header.php");
require_once('inc.school/school_validate.php'); // IMP
...
if($_GET['assesment_id'] > 0) {
    ...
    unset($_SESSION['ses_school_list']); // qn_list
   ...
   //  $sql="SELECT * FROM assessments WHERE id=".$_GET['assesment_id'];
    ...
    while($asses = mysql_fetch_assoc($edit_assessment)) {
        $_SESSION['ses_school_list'][] = $asses['qn_id'];
        // new list prepare//
                ...
                <div id="list-document" class="content_wrap">
                    <form method="post">
                        <div class="ct_heading clear">
                            ...
                            <button type="button" id="removeall-in-list" name="removeall_in_list">Remove All</button>
                        </div>		<!-- /.ct_heading -->
...
if (mysql_num_rows($childs) > 0) {
    ...
    while ($item = mysql_fetch_assoc($childs)) {
        ...
        <?php echo $echo; ?>
                                        ...
                                        <button type="button" class="remove-in-list" name="remove_in_list" value="<?php echo $item['id']; ?>"><i class="fa fa-times"></i>  Remove</button>			
                                    </li>
        <?php
        $i++;
    }
...
?>
                    ...
                    <script>
                        ...
                        // Remove all Questions of assessment 
                        $(document).ready(function () {
                            ...
                            $('#removeall-in-list').on('click', function () {
                             ...
                             //   $.post("inc/ajax-remove-in-school.php", {"removeall_in_list": $boo});
                              $.post("inc.school/ajax-remove-in-school.php", {"removeall_in_list": $boo},
                              function(data, status){
                          if(status=="success"){
                        //  alert( "\nStatus: " + status); 
         location.href='school_assessment_step3.php';}
                               }
                              );
                            ...
                            $('.remove-in-list').on('click', function () {
                                ...
                                /*remove id in list*/
                                var $id = $(this).val();
                                $.post("inc.school/ajax-remove-in-school.php", 
                                {"remove_in_list": $id}, function(data, status){
       // alert("Data: " + data + "\nStatus: " + status);  
         ...
         location.href='school_assessment_step3.php';
                            ...
                            $("#ul-list").sortable({
                                ...
                                update: function (event, ui) {
                                    ...
                                    /* alert(sort); */
                                    $.post("inc.school/ajax-sort_id.php", {"sort": sort});
