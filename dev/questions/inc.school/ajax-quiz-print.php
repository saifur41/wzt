 ...
 * **/
 ...
 #####################
	...
	if( !(isset($_SESSION['login_user'])&&isset($_SESSION['login_id'])&&isset($_SESSION['login_role'])) && !$_SESSION['demo_user_id']) {
...
if($_SESSION['login_role']==1&&!isset($_SESSION['list'])){ //Teacher
...
$header_title=(isset($_SESSION['ses_lesson_name']))?$_SESSION['ses_lesson_name']:'Lesson Quiz';
...
$header = '
	<htmlpageheader name="main">
		<div class="ct_heading clear">
			<div class="underline">
				<h3 style="text-align:center;">'.$header_title.' <sup>TM</sup></h3>
			</div>
		</div>
	</htmlpageheader>';
...
$instructionStop= '<div class="underline">
		...
		<div>
			...
			<div class="instructions-example-item"><strong>5. Target Intervention:</strong> Break students into small groups based on distractor.
				<div><i>Intervene can provide additional targeted concept questions and tutorials/ interventions to teach, reinforce, and verify student mastery of identified concept gaps.</i></div>
			</div>
		...
		if($_GET['for'] != 'teacher') {
			...
			if( mysql_num_rows($childs) > 0 ) {
				...
				while( $item = mysql_fetch_assoc($childs) ) {
					if( $item['type'] == 1 ) {
						...
						foreach( $answers as $key => $answer ) {
							...
							$String = '<td style="width:20px; vertical-align:top;">' . $bulet[$j] . '.</td>';
				...
				// Include section if current logged in user is test@lesstestprep.com
					...
					// Distractor & Misconception Analys
					if($_GET['for'] != 'teacher'&&isset($Stop)) {
                                           //$content.=''; 
					$content .='<pagebreak />
...
<?php
	...
	// if(isset($_POST['print'])&&isset($_POST['print-content'])){
		...
		$mpdf->SetHTMLFooter('<div class="mpdf-footer">Intervene, LLC &copy; 2012-'.date("Y").'</div>');
		...
		$mpdf->WriteHTML($html);
		$mpdf->Output('ExitQuizQuestions.pdf','D');
		/////
