...
////Teacher: with empty question list//
if($_SESSION['login_role']==1&&!isset($_SESSION['ses_school_list'])){ //Teacher
   // exit('No question to print'); HTTP_REFERER
   // https://intervene.io/questions/save&print.php
...
if (isset($_SESSION['ses_school_list'])) {
    ...
    if ($clause == "") {
        $in = " WHERE q.`id` IN ( '" . implode($_SESSION['ses_school_list'], "', '") . "' )";
    } else {
        $in = " AND q.`id` IN ( '" . implode($_SESSION['ses_school_list'], "', '") . "' )";
    }
    $orderby = " ORDER BY FIELD(q.`id`,'" . implode($_SESSION['ses_school_list'], "', '") . "')";
} else {
    if ($clause == "") {
        $in = " WHERE q.`id` IN ( '' )";
    } else {
...
$i = 1;
 ...
 //echo 'passage in linst==='.mysql_num_rows($passage_in_list); die;
// List of questions. w/o Passage///
// mysql_query
$childs = ("SELECT q.*, GROUP_CONCAT(t.`name` SEPARATOR '|') AS terms FROM `questions` q " . $innerjoin ." AND t.`taxonomy`='objective'" . $clause . $in . $groupby. $orderby);
...
if( mysql_num_rows($childs) > 0 ) {
				...
				// if( $user_email == 'test@lesstestprep.com' ) {
					...
					if($_GET['for'] != 'teacher') {
                                           //$content.=''; 
					$content .='<pagebreak />
					<h2 style="text-align: center" >Distractor & Misconception Analysis <sup>TM</sup> </h2><br/>
                                        <table id="tab-answer">
						<thead>
							<tr>
								<th>Question</th>
								<th>Objective</th>
								<th>Answer</th>
								<th>Distractor Analyses Table <sup>TM</sup></th>
							</tr>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
				...
				// if( $user_email == 'test@lesstestprep.com' ) {
					...
					$content .='</tbody></table>';
				// }
			} else { // no question w/o passage.
				$content_hint= '<div class="item-listing clear"><p>There is no item found!</p></div>';
			}
...
if (mysql_num_rows($passage_in_list) > 0) {
       ...
       // echo  $childs ; die; 
        ...
        if (mysql_num_rows($childs) > 0) {
            ...
            while ($item = mysql_fetch_assoc($childs)) {
                ...
                array_push($array, $tmp);
            } // Cal :distractor array
           // echo 'DistractorArrray================<br/>';
            //print_r($array);  die;
         //  print_r($lastrow.'=========='.$numResults) ; die;
          ...
          ///  if ($lastrow == $numResults) {
                // student test
               $content .= '<pagebreak />';// distractor on next page.
                ...
                //$content .= '<div id="text-after-question"><strong>X1--Which Distractor, if any, did you choose the most?  ___________________<strong></div>';
           // }
            
            
            // 2nd times : distractor sectionf 
    ...
    $content .= '<div class="item-listing clear"><p>There is no item found!</p></div>';// Passage
}
$content .= '
	</div>
</div>';
...
/*
  if(isset($_SESSION['ses_school_list']))unset($_SESSION['ses_school_list']);
 
 */
if($_SESSION['login_role']==1){ //Teacher
if(isset($_SESSION['ses_school_list']))unset($_SESSION['ses_school_list']);
