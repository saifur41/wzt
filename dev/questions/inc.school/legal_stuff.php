...
/////////////////
function validate_legal_state_redirect(){
	echo 'Validate URL 1, 2 or 3 form move for a useer==';
	$profile_det=mysql_fetch_assoc(mysql_query("SELECT id,legal_curr_state,is_legal_1,is_legal_2,is_legal_3 FROM `tutor_profiles` WHERE tutorid=".$_SESSION['ses_teacher_id']));
	@extract($profile_det);
 print_r($is_legal_1);
 ///Case 1 
 if($is_legal_1==1&&$is_legal_2==1&&$is_legal_3==1){
 	//move to training 
 	$url='training.php';

 }elseif($is_legal_1==1&&$is_legal_2==1&&$is_legal_3==0){ // first 2 done 
 	  $url='legal_stuff_form3.php'; //step 3

 }elseif($is_legal_1==1&&$is_legal_2==0&&$is_legal_3==0){ // first 2 done 

 	 $url='legal_stuff_form2.php'; //step 3
 }
 /////////
  return  $url;
...
// $profile_det=mysql_fetch_assoc(mysql_query("SELECT * FROM `tutor_profiles` WHERE tutorid=".$_SESSION['ses_teacher_id']));

// print_r($profile_det);

##########################Application state arr########################################
if(isset($_POST['Request'])&&$_POST['is_sign_done']==1){
 //validate_legal_state_redirect();
 $profile_det=mysql_fetch_assoc(mysql_query("SELECT id,legal_curr_state,is_legal_1,is_legal_2,is_legal_3 FROM `tutor_profiles` WHERE tutorid=".$_SESSION['ses_teacher_id']));
 ...
 //print_r($profile_det);
  ...
  //////////Save//////////////////
    ...
    $profile=mysql_query("SELECT * FROM `tutor_profiles` WHERE is_legal_1=1 AND is_legal_2=1 AND is_legal_3=1 AND tutorid=".$_SESSION['ses_teacher_id']);
      ...
      //$data=mysql_fetch_assoc($profile); //print_r($data); die; 
