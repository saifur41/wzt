...
/***
https://intervene.atlassian.net/browse/IIDS-145
@ Mark inactive option for asseement..
...
// as_id
if(isset($_GET['as_id'])&&isset($_GET['mark'])){
  $getid=$_GET['as_id'];
  $sql_update=mysql_query(" UPDATE  assessments SET status_active='no' WHERE id=".$getid);

   if($sql_update){
    $error='Assessment ID-'.$getid.', Mark as inactive! ';
    $RedirectPage=1;
   }
...
ac.entity_id IN ('.$ent_id_lsX.')
   ...
   // $search_sql=' SELECT a.* FROM assessments a WHERE 1 ';
      // $search_sql=' SELECT DISTINCT(a.id) FROM assessments a WHERE 1 AND status_active';

       $search_sql=" SELECT DISTINCT(a.id) FROM assessments a WHERE 1 AND status_active='yes' ";
...
<?php } ?>
                ...
                <?php  
                  ...
                  $childs = mysql_query($search_sql);       
                     ...
                     $childs = mysql_query(" SELECT * FROM assessments WHERE status_active='yes' ORDER BY id deSC ");       
                     }
                     //
                ...
                <form id="form-manager" class="content_wrap" action="" method="post">
					<div class="ct_heading clear">
						<h3>Assesment List(#<?=$tot_records;?>)</h3>

            <a href="assesment_list_all.php" class="btn btn-danger btn-xs">View all</a>
                            ...
                            ?>       
                                      ...
                                      <a   title="Edit" href="edit_assesment.php?assesment_id=<?=$row['id']?>">
                                          ...
                                          <span class="fa fa-copy"></span></a> 
                                                          ...
                                                          <br/>
                                                          <a href="assesment_list.php?as_id=<?=$row['id'].'&mark=inactive'?>" 
                                                          class="btn btn-danger btn-xs">Mark Inactive</a>             
...
<?php  if(isset($RedirectPage)){ // RedirectPage Required?>
   <script type="text/javascript">
     window.location.href = "assesment_list.php";
