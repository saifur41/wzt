                          ...
                          $result=mysql_query($sql);
                               ...
                               if (mysql_num_rows($result) > 0) {
                                   ...
                                   while ($row = mysql_fetch_assoc($result)) {
                                    ...
                                    $str='SELECT id,CONCAT(first_name, " ", last_name) AS fullname FROM `students` WHERE id='.$row['student_id'];
                                     $sql_stud=mysql_fetch_assoc(mysql_query($str));
...
GROUP BY s_x_s.student_id HAVING percentage < 50 ');
        while ($students = mysql_fetch_assoc($less_50_stu)) {
            ...
            $str='SELECT id,CONCAT(first_name, " ", last_name) AS student_name FROM `students` WHERE id='.$students['student_id'];

            $student_name = mysql_fetch_assoc(mysql_query($str));
