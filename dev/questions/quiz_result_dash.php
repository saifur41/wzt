<?php
/***
@ small_group_summary LINK
// https://intervene.io/questions/quiz_result_dash.php?cid=430&quizid=121&ses_id=2383&add_class=Get+Data+Dash
**/
@session_start();
...
if ($_GET['quizid']) {
///////**********SCORE SUMMARY*********/////////mysql_query



  $get_ses_id=$_GET['ses_id'];
  
    // Issue with Pay Pal Mass Payout function mysql_query
    $score_res=mysql_query("SELECT SUM( corrected ) AS correct, count( qn_id ) AS total, ((SUM( corrected ) / count( qn_id )) *100) AS percentage, student_id FROM students_x_quiz WHERE quiz_id='".$_GET['quizid']."' AND class_id = '".$_GET['cid']."' GROUP BY student_id");

    if(isset($get_ses_id)){

        $score_res=mysql_query("SELECT SUM( corrected ) AS correct, count( qn_id ) AS total, ((SUM( corrected ) / count( qn_id )) *100) AS percentage, student_id FROM students_x_quiz WHERE quiz_id='".$_GET['quizid']."' AND class_id = '".$_GET['cid']."' AND tutses_id= '".$get_ses_id."'  GROUP BY student_id");

    }

  
    //print_r('T:'.$score_res); die; 
   ...
   ################### 
    ...
    /// REDINESS SUMMARY mysql_query
     ...
     //        . 'WHERE a_x_s.assesment_id = \'' . $_GET['quizid'] . '\'  AND t.objective_type = "R" GROUP BY t.id ');
    // quiz_id and  session_id  mysql_query
    $readiness_res =mysql_query('SELECT t.id, t.objective_type, t.short_code, t.obj_short, t.name FROM terms t '
            . 'LEFT JOIN term_relationships rel ON rel.objective_id = t.id '
            . 'LEFT JOIN int_quiz_x_questions a_x_s ON a_x_s.qn_id = rel.question_id '
            . 'WHERE a_x_s.quiz_id = \'' . $_GET['quizid'] . '\'  AND t.objective_type = "R" GROUP BY t.id ');

   // print_r($readiness_res); die; 
...
FROM students_x_quiz s_x_s 
LEFT JOIN term_relationships rel ON rel.question_id = s_x_s.qn_id 
AND rel.objective_id = \'' . $readiness_result['id'] . '\'  
WHERE s_x_s.quiz_id=\'' . $_GET['quizid'] . '\'  AND s_x_s.class_id = \''.$_GET['cid'].'\'  AND  rel.question_id = s_x_s.qn_id 
GROUP BY s_x_s.student_id');

        // print_r($readinesss_score_res); die; 
...
//die;
    ...
    $students = array(); // mysql_query
    $readiness_res =mysql_query('SELECT t.id, t.objective_type, t.short_code, t.obj_short, t.name FROM terms t '
            . 'LEFT JOIN term_relationships rel ON rel.objective_id = t.id '
            . 'LEFT JOIN int_quiz_x_questions a_x_s ON a_x_s.qn_id = rel.question_id '
            . 'WHERE a_x_s.quiz_id= \'' . $_GET['quizid'] . '\'  AND ( t.objective_type = "R" OR t.objective_type = "S" ) GROUP BY t.id ');
...
GROUP BY s_x_s.student_id');
        ...
        $percentage_student_obj_stop=" SELECT SUM( s_x_s.corrected ) AS correct, count( s_x_s.qn_id ) AS total, ((SUM( s_x_s.corrected ) / count( s_x_s.qn_id )) *100) AS percentage, student_id FROM students_x_quiz s_x_s LEFT JOIN term_relationships rel ON rel.question_id = s_x_s.qn_id AND rel.objective_id = '791' WHERE s_x_s.quiz_id='119' AND s_x_s.class_id = '430' AND rel.question_id = s_x_s.qn_id GROUP BY s_x_s.student_id";
    ...
    $total_mis = 0;//mysql_query
    $res =mysql_query('SELECT count(DISTINCT s_x_a.student_id) as cnt, s_x_a.distractor, d.name FROM students_x_quiz s_x_a '
            . 'INNER JOIN distrators d ON s_x_a.distractor = d.id '
            . 'WHERE s_x_a.corrected = 0 AND s_x_a.distractor > 0 '
            //. 'AND s_x_a.tutses_id = \'' .$get_ses_id. '\' '
            . 'AND s_x_a.quiz_id = \'' . $_GET['quizid'] . '\' AND s_x_a.class_id = \''.$_GET['cid'].'\'  ' 
            . 'GROUP BY  s_x_a.distractor ORDER BY count(DISTINCT s_x_a.student_id) DESC LIMIT 0,10  '); // 0,5

                   #XXX
                //echo 'distrators part:<br/>';
    ...
    while ($mis = mysql_fetch_assoc($res)) {
        ...
        // quiz_id , tutses_id
        $mis_num_sql='SELECT s_x_a.num, t.obj_short FROM students_x_quiz s_x_a '
                . 'INNER JOIN distrators d ON s_x_a.distractor = d.id '
                ...
                . 'AND s_x_a.quiz_id= \'' . $_GET['quizid'] . '\' AND s_x_a.class_id = \''.$_GET['cid'].'\'  AND s_x_a.distractor = \'' . $mis['distractor'] . '\'  ';

                //  session quiz wise report.
                // if(isset($get_ses_id)){
                //     $mis_num_sql.=' AND s_x_a.tutses_id = \'' .$get_ses_id. '\' ';
                // }

            $mis_num=mysql_query($mis_num_sql);   
 ...
 $get_quiz_id=$_GET['quizid'];
// default
  $sql="SELECT COUNT(distractor), distractor, student_id FROM students_x_quiz WHERE distractor > 0 "
        . " AND teacher_id=516 AND assessment_id= 3 GROUP BY distractor, student_id HAVING COUNT(distractor) >=1 ORDER BY distractor ";
  
  // case-2 :: quiz_id, tutses_id
  $sql="SELECT COUNT(distractor), distractor, student_id FROM students_x_quiz WHERE distractor > 0 "
        . " AND class_id='$class_id' AND teacher_id='$user_id' AND quiz_id='$get_quiz_id' ";

        // Session wise report
       if(isset($get_ses_id)){
        $sql.=" AND tutses_id=".$get_ses_id;

       }
        

        $sql.=" GROUP BY distractor, student_id HAVING COUNT(distractor) >=1 ORDER BY distractor  ";
                               ...
                               if (mysql_num_rows($result) > 0) {
                                   ...
                                   while ($row = mysql_fetch_assoc($result)) {
                                    ...
                                    //  $sql_stud=
                                     ...
                                     $sql_stud=mysql_fetch_assoc(mysql_query('SELECT id,CONCAT(first_name, " ", middle_name, " ", last_name) AS fullname FROM `students` WHERE id='.$row['student_id']));
...
<html>
    <head>
        <title>Interventions Exit Quiz &trade;</title>
...
<?php }else { $message = 'There were no readiness objectives selected on this Quiz.'; ?>
<?php } ?>
        <style>
   ...
   <tr class="">
    <td style="border:0px;height:50px;font-weight: bold; font-size: 30px;" colspan="2" align="center"><p style="margin-bottom: 55px;">Teacher Exit Quiz <sup>TM</sup></p></td></tr>
    </thead> 
...
<td  style="border-left:0px !important;border-right:0px !important;font-size: 12px;">' . $value['short_code'] . '</td></tr>';
    ...
    $html .= '
            ...
            <tr>
             ...
             //  print_r($small_group_summary); die; 
             

               // print_r($less_50_stu_sql); die; 
    ...
    foreach ($small_group_summary as $k => $v) {
       ...
       // echo 'small_group_summary ====';  die; 
        ...
        $less_50_stu_Stop=('SELECT SUM( s_x_s.corrected ) AS correct, count( s_x_s.qn_id ) AS total, 
            ((SUM( s_x_s.corrected ) / count( s_x_s.qn_id )) *100) AS percentage, student_id
FROM students_x_quiz s_x_s 
LEFT JOIN term_relationships rel ON rel.question_id = s_x_s.qn_id 
AND rel.objective_id = \'' . $k . '\'  
WHERE s_x_s.assessment_id =\'' . $_GET['quizid'] . '\' AND s_x_s.class_id = \''.$_GET['cid'].'\' AND  rel.question_id = s_x_s.qn_id 
GROUP BY s_x_s.student_id HAVING percentage < 50 ');

        //////////////////###////////////


         $less_50_stu_sql='SELECT SUM( s_x_s.corrected ) AS correct, count( s_x_s.qn_id ) AS total, 
            ((SUM( s_x_s.corrected ) / count( s_x_s.qn_id )) *100) AS percentage, student_id
FROM students_x_quiz s_x_s 
LEFT JOIN term_relationships rel ON rel.question_id = s_x_s.qn_id 
WHERE s_x_s.quiz_id=\'' . $_GET['quizid'] . '\' AND rel.objective_id = \'' . $k . '\' AND s_x_s.class_id = \''.$_GET['cid'].'\' AND  rel.question_id = s_x_s.qn_id 
';

    $less_50_stu_sql.=' AND s_x_s.tutses_id = \''.$get_ses_id.'\' ';

    $less_50_stu_sql.=' GROUP BY s_x_s.student_id HAVING percentage < 50 ';


    $less_50_stu = mysql_query($less_50_stu_sql);
