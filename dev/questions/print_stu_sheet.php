...
$teacher_id=$user_id = $_SESSION['login_id'];
...
//////////////////
 $ses_id=$_GET['ses'];
  $sql_student=" SELECT sx.slot_id,sx.student_id,s.id as sid,s.first_name,s.middle_name,s.last_name FROM int_slots_x_student_teacher sx
left join students s
ON sx.student_id=s.id
WHERE sx.slot_id= '$ses_id' AND sx.int_teacher_id='$teacher_id' "; 

 // $res = ('SELECT first_name, middle_name, last_name, username, password, dob FROM students WHERE '
...
if(isset($_GET['ses'])) {
    ...
    $class_id = $_POST['class']; // mysql_query
    ...
    $res=mysql_query(" SELECT sx.slot_id,sx.student_id,s.id as sid,s.* FROM int_slots_x_student_teacher sx
left join students s
ON sx.student_id=s.id
WHERE sx.slot_id= '$ses_id' AND sx.int_teacher_id='$teacher_id' "); 
...
<body><table width="100%" border=1><tr><td align="center" colspan="3"><h1>Intervention: '.$ses_id.' <br/> Student List</h1></td></tr>
';
