...
/* add grade*/
if($action=='addGradeWrite')
{

	$str="UPDATE telpas_student_writing_grades SET `scored_grade_number`='".$score."' WHERE  `id`=".$id;
	if(mysql_query($str))
	{
		...
		$str="UPDATE telpas_student_writing_grades SET `scored_grade_number`='".$score[$key]."' WHERE  `id`=".$row;
	...
	if(mysql_query($str))
	{
		$str="SELECT * FROM `telpas_student_writing_grades`WHERE course_id='".$course_id."' AND intervene_student_id='".$student_id."' ORDER BY `question_id`"; 
		$audio=mysql_query($str);
		while ($row = mysql_fetch_assoc($audio)) 
		{
