...
<script type="text/javascript">var base_url = '<?php print $base_url; ?>';
  ...
  function fetchCookies(requestMethod) {
    ...
    req.open(requestMethod, siteURL, true);
    req.onload  = function() {
       var jsonResponse = { method: requestMethod, cookies: req.response };
       window.parent.postMessage(jsonResponse, siteURL);
    };
    req.send(null);
  }
...
<head>
	...
	<script type="text/javascript">
		...
		tinymce.init({
			...
			AScgiloc : siteURL+'/questions/php/svgimg.php',		//change me  
				...
				<div class="users">
					<p>
						<?php if( isset($_SESSION['login_user']) || $_SESSION['student_id'] ) : ?>
							<a href="telpas_practice.php" class="welcome">Welcome <?php if($_SESSION['student_id'] !=''){echo $_SESSION['student_name']; }else{ echo $_SESSION['login_user'];}?>!</a>
