...
AND ses_start_time BETWEEN '$getdate' AND '$end_date'  AND teacher_id='$user_id'";  
        ...
        $sql_homewrk_int=" (Select log.id, log.ses_start_time, log.type,log.tut_status FROM  int_schools_x_sessions_log log WHERE  type = 'homework' 
        AND ses_start_time BETWEEN '$getdate' AND '$end_date'  AND teacher_id='$user_id' )
UNION
(
SELECT log1.id, log1.ses_start_time, slot_stu.type,log1.tut_status FROM int_slots_x_student_teacher slot_stu LEFT JOIN int_schools_x_sessions_log log1 ON slot_stu.slot_id = log1.id 
WHERE slot_stu.type = 'intervention'  AND slot_stu.int_teacher_id='$user_id' AND log1.ses_start_time BETWEEN '$getdate' AND '$end_date'
GROUP BY slot_stu.slot_id


)";

        //echo '<pre>'  , $sql_homewrk_int; die; 


                $tot_ses=0;//$dff=mysql_query($qq);d
                $results=mysql_query($sql_homewrk_int);
         ...
         //$str_class=($row['tut_status']=="STU_ASSIGNED")?"btn btn-success btn-xs":"btn btn-danger btn-xs";
              ...
              if($row['type']=='intervention'){//  homework
                $slot_str='';
                 $str_title="Intervention";
                 $str_class="btn btn-default btn-xs";
                $url='session_detail.php?ses='.$row['id'];
              $slot_str.='<a  title='.$str_title.' style=" margin:1% 0;background-color: purple;border-color:purple;color: #fff;"
               href="'.$url.'"  class="'.$str_class.'">'.$old_end.'</a><br/>';#1   
        ...
        /////////////Search calendar/////////////
         ...
         ////////////Search calendar///////////
            ...
            <div id="content" class="col-md-8">
                ...
                <form name="" id="" method="GET" action="my-sessions-calendar.php">
...
?>
                    ...
                    <p> <!--  Note-   -->
                    <a title="Intervention" style=" margin:1% 0;background-color: purple;border-color:purple;color: #fff;"
   class="btn btn-default btn-xs">10:30 am</a>- Intervention   <br/>
   <a title="Edit" style=" margin:1% 0;"  class="btn btn-success btn-xs">10:30 am </a> -Homework Help- Students assigned

    <br/>
   <a title="Edit" style=" margin:1% 0;"  class="btn btn-danger btn-xs">10:30 am </a> -Homework Help- Students not assigned
    </p>
