...
if( isset($_POST['question_submit']) ) {
	...
	// print_r(array_unique($_POST['suggest'])); die;
	// echo '</pre>';
	// die();
	...
	if($_POST['translate'] == 1){
		...
		// $question_question_spanish = $_POST['question_question'];
	...
	if( $question_type == 1 ) {
		...
		for( $i=0; $i<4; $i++ ) {
			...
			// Setting image dimension
			$answer_spanish_item['width'] = $width_img[$i];
			$answer_spanish_item['height'] = $height_img[$i];
...
// Do query
	...
	$error	= $return ? 'Successfully!' : 'Can not ' . $alert . ' question. Please try again later!';
