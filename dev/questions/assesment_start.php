...
//print_r($_SESSION['assessment']['qn_list']);
...
if($_POST['complete']) {
    ...
    $str='UPDATE teacher_x_assesments_x_students SET status = "Completed" , completion_date = \''.$created.'\' WHERE '
    . 'student_id = \'' . $_SESSION['student_id'] . '\' AND '
    . 'assessment_id = \''.$assesment_id.'\' ';
    mysql_query($str);
...
if ($_POST['next']) {
   // echo '<pre>';  print_r($_POST); die;
    ...
    $grade_id = $gievn_questions['grade_id'];
        ...
        $str="SELECT * FROM teacher_x_assesments_x_students  WHERE student_id = '".$_SESSION['student_id']."' 
        AND assessment_id ='".$assesment_id."'";

        $classRow = mysql_fetch_assoc(mysql_query($str));

        $ClasiD=$classRow['class_id'];
        ...
        $str='INSERT INTO students_x_assesments SET '
...
$str="SELECT teacher_id FROM teacher_x_assesments_x_students  WHERE student_id = '".$_SESSION['student_id']."' 
AND assessment_id ='".$assesment_id."'";
$techR = mysql_fetch_assoc(mysql_query($str));
$teacher_id=$techR['teacher_id'];



 $str='SELECT * FROM students_x_assesments WHERE '
        . 'student_id = \'' . $student_id . '\'  AND '
         . 'teacher_id = \'' . $teacher_id . '\'  AND '
        . 'qn_id = \'' . $qn_id . '\'  '
        . 'AND assessment_id = \'' . $assesment_id . '\' ';

$stored_res = mysql_query($str);
$stored_result = mysql_fetch_assoc($stored_res);

$str="SELECT * FROM `questions` WHERE id = " . $qn_id;

$childs = mysql_query($str);


//$item = mysql_fetch_assoc($childs);

//($item);
//die;
$str="SELECT DISTINCT `q`.`passage` , `q`.`id` AS qid, p. *
FROM `questions` q INNER JOIN `passages` p ON `q`.`passage` = `p`.`id` WHERE q.`id`IN (".$qn_id.") ";
   $res_passage=mysql_query($str);
   //die; 
