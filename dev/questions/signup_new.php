...
@ Testing:rohit@srinfosystem.com
   ...
   // print_r($_POST);  die; 
    ...
    if (!empty($_POST['signup-email'])) {
        ...
        if ($rows < 1) {
            ...
            $queryxx= "INSERT INTO `users` ( `user_name` , `email` , `password` , `first_name` , `last_name` , `school` , `q_remaining` ) 
                ...
                if (mysql_num_rows($invited) > 0) {
                  ...
                  ///   $school   :: Changed
                   ...
                   // print_r($sharedGrades); print_r($data_dash); die;
                    ...
                    //  Send email to activate : teacher user
                    // Send activate email
                    require_once('inc/function-active-user.php');
                    sendEmailToActive($userId);
                }
