...
$demoUserId=$_SESSION['demo_user_id'];
...
$shared = "SELECT `id`
	...
	@Demo user 
	SELECT * FROM `terms` WHERE `taxonomy` = 'objective' AND (`name` LIKE '%4.%' OR `description` LIKE '%4.%') AND `active` = 1


	**/
	// UserFoldrShared
	$sql_folder=mysql_query(" SELECT id,name,taxonomy,description,parent,active FROM terms WHERE id IN (23,34)" ); //grade shared to demo user 
      $arr_user_grades=array();

      $parent_folders=array();
    while ($line=mysql_fetch_assoc($sql_folder)){
    	$arr_user_grades[]=$line;
    	if($line['parent']>0)
    	array_push($parent_folders, $line['parent']);

    }
    $parent_folders=array_unique($parent_folders);

$DemoUser=mysql_fetch_assoc(mysql_query(" SELECT *
FROM demo_users
WHERE id =".$demoUserId));
// var_export($DemoUser['smart_prep']);die; 
$strSubjectFolderId=$DemoUser['smart_prep'];



    // print_r($parent_folders);
    // print_r($arr_user_grades); die; 

	/////////////////////

 $searchFolder = mysql_query("SELECT * FROM `terms` WHERE `taxonomy` = 'category' AND `name` LIKE '%{$s}%' AND `active` = 1 AND 0"); 
//$searchObjective =("SELECT * FROM `terms` WHERE `taxonomy` = 'objective' AND (`name` LIKE '%{$s}%' OR `description` LIKE '%{$s}%') AND `active` = 1");



$sql_searchObjective ="SELECT * FROM `terms` WHERE `taxonomy` = 'objective' AND (`name` LIKE '%{$s}%' OR `description` LIKE '%{$s}%') AND `active` = 1";

  //if(isset($strSubjectFolderId)&&!empty($strSubjectFolderId))
 $sql_searchObjective.=" AND subject_grade_id IN (".$strSubjectFolderId.") ";  #Purchase folder by DemoUser


  //echo '==' , '<pre>' , $sql_searchObjective; die;

  

/////News search /////////
  $searchObjective=mysql_query($sql_searchObjective);
...
**/
$sharedDemoGrade=$strSubjectFolderId;
  

 if(mysql_num_rows($searchObjective)>0){ // ObjectivesSearch Count>0

$searchQuestion =mysql_query("SELECT * FROM `questions` WHERE (`name` LIKE '%{$s}%' OR `question` LIKE '%{$s}%') AND `category` IN ( {$sharedDemoGrade} ) AND `public` = 1" . $notin);

  }

  //echo 'searchQuestion==' , '<pre>' , $searchQuestion; die;



// AND `q`.`public` = 1 AND `q`.`smart_prep` = 1
		...
		<?php else: ?>
			<div id="result-tabs">
				...
				<?php endif;?>
				  ...
				  <!-- ObjectiveURL: href="objective.php?taxonomy=" -->
							...
							while( $item = mysql_fetch_assoc($searchQuestion) ) {
								if( $item['type'] == 1 ) {
									...
									foreach( $answers as $key => $answer ) {
										...
										$echo .= ($answer['image'] != '') ? '<p class="test"><img src="' . $answer['image'] . '" height="100" /></p>' : '';
