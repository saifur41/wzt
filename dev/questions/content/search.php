...
$searchQuestion =mysql_query("SELECT * FROM `questions` WHERE (`name` LIKE '%{$s}%' OR `question` LIKE '%{$s}%') AND `category` IN ( {$shared} ) AND `public` = 1 AND `smart_prep` = 1 " . $notin);
 
//echo '<pre>';
//print_r($searchQuestion); die;
