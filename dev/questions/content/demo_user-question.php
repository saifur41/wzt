...
if( $select ) {
	...
	$clause = ( $select['taxonomy'] == 'objective' )
			? " INNER JOIN `term_relationships` r ON q.`id` = r.`question_id`
				...
				AND q.`category` IN (
					SELECT `termId` FROM `shared` WHERE `userId` = {$_SESSION['demo_user_id']}
				)"
...
if( $select['taxonomy'] == 'category' ) {
	$validate = mysql_query("SELECT * FROM `shared` WHERE `userId` = {$_SESSION['demo_user_id']} AND `termId` = $termId");
...
// Public Questions
 //$public_question =" AND `q`.`public` = 1 "; 
$public_question =" AND `q`.`public` = 1 AND `q`.`smart_prep` = 1 "; 
...
<div id="list-document" class="content_wrap">
	<div class="ct_heading clear">
		<h3>Add Questions in Document<?php //=mysql_num_rows($childs)?></h3>
	</div>		<!-- /.ct_heading -->
