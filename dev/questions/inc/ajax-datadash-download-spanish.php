    ...
    for ($l = 0; $l < count($_SESSION['list']); $l++) {
        ...
        if (mysql_num_rows($passage_in_list) > 0) {
            ...
            while ($item_passage = mysql_fetch_assoc($passage_in_list)) {
                ...
                $string = str_replace('\"', '"', $item_passage['content'.$lang]);
                        $string = str_replace('&nbsp;',"",strip_tags($string));
                        $string = preg_replace('/\s+/', '', $string);
                        if($string==''){


                                $item_passage['content'.$lang]='Not Translate';
                            }
                ...
                if (mysql_num_rows($childs) > 0) {
                    ...
                    $string = str_replace('\"', '"', $item['question'.$lang]);
                        $string = str_replace('&nbsp;',"",strip_tags($string));
                        $string = preg_replace('/\s+/', '', $string);
                        if($string==''){


                                $item['question'.$lang]='Not Translate';
                            }
