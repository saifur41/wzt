...
/***
@passage question Print
ajax-quiz-passage-print.php
...
$header = '
	<htmlpageheader name="main">
		<div class="ct_heading clear">
			<div class="underline">
				<h3 style="text-align:center;">'.$header_title.'<sup>TM</sup></h3><br/>
			</div>
		</div>
	</htmlpageheader>';
...
$instructionxxxStop = '<div class="underline">
...
if (mysql_num_rows($passage_in_list) > 0) {
    ...
    while ($item_passage = mysql_fetch_assoc($passage_in_list)) {
        ...
        if (mysql_num_rows($childs) > 0) {
            ...
            if ($lastrow == $numResults&&isset($Stop)) {
                // student test
                $content .= '<pagebreak />';
            ...
            if ($lastrow == $numResults&&isset($Stopthis)) {
                // teach answer key
                $content .= '<pagebreak />';
