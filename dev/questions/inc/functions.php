...
function remove_items() {
	...
	if( $type == 'category' || $type == 'objective' ) {
		...
		$query = "DELETE FROM `lessons` WHERE `id` IN ( {$ids} )";
            ...
            mysql_query($query);	
	    $query = "DELETE FROM `assessments_x_questions` WHERE `assesment_id` IN ( {$ids} )";
            mysql_query($query);
            $query = "DELETE FROM `teacher_x_assessments` WHERE `assesment_id` IN ( {$ids} )";
            mysql_query($query);
            $query = "DELETE FROM `teacher_x_assesments_x_students` WHERE `assessment_id` IN ( {$ids} )";
            mysql_query($query);
            $query = "DELETE FROM `students_x_assesments` WHERE `assessment_id` IN ( {$ids} )";
            mysql_query($query);
        }
