...
// echo 'Objextive Pagenation ==========,<br/>';


// Setup $array number of items per page
$numbers = array(10, 20, 50,100,200);
...
//////////If sapnish fileter exits ///////////////////////
 ...
 if(isset($_GET['subject_filter'])&&!empty($_GET['subject_filter'])){
   ...
   $getTax = 'subject_filter='.$_GET['subject_filter'].'&';
 ...
 ///Both Exitst ////
if(isset($_GET['taxonomy'])&& $_GET['taxonomy']>0&&isset($_GET['subject_filter'])&&!empty($_GET['subject_filter']) ){
	$getTax = 'taxonomy='.$taxonomy.'&subject_filter='.$_GET['subject_filter'].'&'; 
	//$getTax = 'subject_filter='.$_GET['subject_filter'].'&';
...
<div class="pagination clear fullwidth">
         ...
         <?php  if(isset($_GET['subject_filter'])&&!empty($_GET['subject_filter'])){ 
		...
		<?php
			...
			//////Field Spanish//
			if(isset($_GET['subject_filter'])&&!empty($_GET['subject_filter'])){
				$langVal=$_GET['subject_filter'];

				echo '<input type="hidden" name="subject_filter" value="'.$langVal.'">';
			}
