...
$roster=mysql_fetch_assoc(mysql_query("SELECT * FROM school_master_rosters WHERE id=".$roster_id));
  ...
  // $roster=mysql_fetch_assoc(mysql_query("SELECT * FROM school_master_rosters WHERE id=".$roster_id));
      ...
      $d2=mysql_query(" DELETE FROM classes WHERE roster_id=".$getid);
        // Delete old classes & Students Entry.
        ...
        if (($handle = fopen($uploads_dir . '/' . $name, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                // print_r($data);  die;
                    ...
                    $first_name = trim($data[6]);
                    $middle_name = trim($data[7]);
                    $last_name = trim($data[8]);
                  ...
                  if(mysql_num_rows($user_sql)==1){ 
                      ...
                      // teacher_id  trim($data[4]);
                       $user_id=$teacher['id'];  // echo '==teacher id'; die;

                     
                    ////////Add Student Data////////

                $grade=trim($data[2]);
              $grade_data=mysql_fetch_assoc(mysql_query("SELECT name FROM terms WHERE id=" . $grade));

                $grade_name= $grade_data['name']; //'Demos';

               // $user_id=trim($data[4]); // teacher_id
               
                
                $class_name=trim($data[1]);  // mysql_query
                // class_code
                $class_code=trim($data[0]);
                  //  validate Class 
  $sql="SELECT * FROM classes WHERE class_code= '".$class_code."' AND grade_level_id=".$grade." AND teacher_id=".$user_id; // die;
     $res_sql=mysql_query($sql);
       if(mysql_num_rows($res_sql)==1){  //update
        ...
        //print_r($class_data); die;
         ...
         $class_i =mysql_query('INSERT INTO classes SET '
                ...
                $UID=trim($data[5]);
                 ...
                 $student=mysql_query('INSERT INTO students SET '
