...
elseif($session_type=="dateBy"){
                               

            $endDate1 = date('Y-m-d', strtotime($endDate . ' +1 day'));

            $sql_ses=" SELECT * FROM int_schools_x_sessions_log where ";
            $sql_ses.="ses_start_time between '".$startDate."' and '".$endDate1."'";
		...
		<td>
                    ...
                    <select name="session_type" id="filterSelect">
                     <option value="all" <?php echo (isset($session_type)&&$session_type=="all")?'selected':NULL; ?> >All</option>
                        ...
                        <option value="dateBy" <?php echo (isset($session_type)&&$session_type=="dateBy")?'selected':NULL; ?>>Date By Sessions</option>
                                ...
                                <div class="dateField">
                            <input type="date" name="startDate" value="<?php echo $startDate?>" placeholder="Start Date">
...
<a title="Join as observer Newrow Room" target="_blank" href="https://tutorgigs.io/myadmin/add-observer.php?roomID=<?php echo $newrow_room_id['newrow_room_id']?>" class="btn btn-danger btn-xs">Join as observer Newrow Room</a>
                              ...
                              <?php }?>
                              <br/> <br/>
              <?php $mod_url="moderator_join_room.php?sesid=".$row['id']; ?>
                              <a target="_blank" class="btn btn-danger btn-xs" 
                               href="<?=$mod_url?>"
                               >Join as instructor</a>
                                 <br/><br/>
                                </td>
                               ...
                               <a href="javascript:void(0);" 
                                ...
                                >Virtual board:<?=ucfirst($row['board_type']) ?> </span> <br/>
                                           ...
                                           <?=date_format(date_create($row['created_date']), 'F d,Y');?> 
                                                ...
                                                <br/>
                                          <strong class="text-primary">Session:</strong>session_type<br/>       
...
<script type="text/javascript">
  ...
  $('#filterSelect').change(function(){
    ...
    var opt =$(this).val();
    if(opt=='dateBy'){
      $('.dateField').addClass('filedcls');
    }
    else{

      $('.dateField').removeClass('filedcls');
    }
  });
</script>
