...
$qq = " SELECT tutor_profiles.* FROM gig_teachers inner join tutor_profiles on (gig_teachers.id = tutor_profiles.tutorid) WHERE `all_state` = 'Yes' ";
$qq_result = mysql_query($qq);

$total_warning = 0;
while ($data = mysql_fetch_assoc($qq_result)) {
     if(!empty($data['teacher_certificate']) && is_null($data['teacher_approve']))
                                                {
                                                    $total_warning++;
                                                }
                                                else if(!empty($data['esl_certificate']) && is_null($data['esl_approve']))
                                                {
                                                    $total_warning++;
                                                }
                                                else if(!empty($data['billingual_certificate']) && is_null($data['bilingual_approve']))
                                                {
                                                    $total_warning++;
...
$ses_sql = mysql_query("SELECT * FROM `int_schools_x_sessions_log` WHERE observer_confirm = 1 and add_observer = 1");
$ses_count = mysql_num_rows($ses_sql);
?>
...
<div id="objective" class="widget clear fullwith">
	<?php include('widget/widget-sessions.php');?>
</div>		<!-- /#objective -->
<?php  // endif;?>
...
<div id="objective" class="widget clear fullwith">
	<h4 class="widget-title"><i class="fa fa-paragraph"></i>Test Qualification</h4>
<div class="widget-content">
	<p class="list"><i class="fa fa-th-list"></i><a href="https://tutorgig.englishpro.us/test_engine">View</a></p>
        <p class="list"><i class="fa fa-th-list"></i><a href="training_management.php">Trainig Management</a></p>
</div></div>

<div id="objective" class="widget clear fullwith">
    <h4 class="widget-title"><i class="fa fa-paragraph"></i><a href="tutor-certificate-list.php">Certification List </a> <span class="badge" style="background-color:red"><a href="pending_certificate_tutor.php" style="color:#fff"><?php echo $total_warning;?></a></span></h4>
</div>
