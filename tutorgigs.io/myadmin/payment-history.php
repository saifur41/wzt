<?php
    /**
    @List of Tutors - that have completed sessions & feedback
    */
    //
    include("header.php");
    ...
    if (isset($_POST['submit_mark_paid'])) {
        ...
        if ($arr != "") {
            $query = mysql_query("UPDATE int_schools_x_sessions_log 
                SET 
                payment_status='1',
                payment_date='$curr_time' 
                WHERE id IN ($arr)", $link);
        }
    ...
    if (isset($_POST['paybypaypal'])) {  
        ...
        if ($arr != "") {    
            ...
            $insert_batch=mysql_query(" INSERT 
                INTO ses_payout_batchs 
                SET batch_ses_ids = '$arr',
                created_at = '$curr_time' "
            );
            $curr_batch_id= mysql_insert_id(); //  $curr_batch_id=99;

            // add batch info ***EMAIL MESSAGE??
            $res['sender_batch_header'] = array(
                'sender_batch_id' => $bach_number = $curr_batch_id,
            ...
            $sql_ses="SELECT 
                ...
                t.payment_em,
                t.email,
                group_concat(ses.id) as ses_ids,
                COUNT(ses.id) as ses_count,
                COUNT(ses.id)*".$session_pay_amount." as payout_amt
            ...
            $sql_ses="SELECT 
                ses.id as sesid,
                ses.tut_teacher_id as ses_tutor,
                t.* 
                FROM int_schools_x_sessions_log ses
                LEFT JOIN gig_teachers t 
                ON ses.tut_teacher_id=t.id
            ...
            ";
            */
            $resultSessionsGroup = mysql_query($sql_ses);

            $data_arr = array();
            
            $item_arr = array();

            while ($line = mysql_fetch_assoc($resultSessionsGroup)) {

                $arr1['id'] = $line['id'];
                $arr1['email'] = $line['email'];
                $arr1['ses_id'] = $line['ses_id'];
                ...
                $item_arr[] = array(
                    ...
                    'amount' => array('value' => $line['payout_amt'], 'currency'=>'USD'),
                    'note' => 'Thanks for your interest!', 
                    'sender_item_id' => $line['id'], 
                    'receiver' => $line['payment_em'], 
                    //'receiver' => $line['payment_em'], // WHY 2X??
                    'sessionID' => $line['ses_ids'], //$ses_id_str,
                );
            ...
            ///////Test////
            //echo '<h3>Total Records = '.$tot_records.'<h3>';
            //echo '<pre>::';
            //print_r($res);   
            //die;  
            // 
            ...
            $sql = mysql_query("UPDATE 
                ses_payout_batchs 
                SET 
                batch_request ='$new_json',
                updated_at = '$curr_time',
                batch_curr_state='$payout_res' 
                WHERE id = ".$curr_batch_id
            );


            if (isset($pay_obj->name) && $pay_obj->name == 'INSUFFICIENT_FUNDS') {
                $warnings[] = $msg = 'INSUFFICIENT_FUNDS,'.$pay_obj->message;
                echo "<script>alert('".$msg."');location.href='payment-history-k.php';</script>";  
                die;
            }
            ...
            if (isset($pay_obj->batch_header->batch_status) && $batchStatus == 'PENDING') { 
                ...
                $sql=mysql_query("UPDATE 
                    ses_payout_batchs 
                    SET 
                    payout_batch_id = '$payoutBatch_id',
...
<script>
    // Check All and Delete User (Mark as paid)
		...
		<div class="row">
            ...
            <?php
                $sql_tutors="SELECT 
                    ...
                    gt.lname
                    FROM 
                    int_schools_x_sessions_log s
                    JOIN 
                    gig_teachers gt ON s.tut_teacher_id = gt.id
                    WHERE 1 AND feedback_id > 0  
                    AND ses_start_time < '".$curr_time."' 
                    AND payment_status='0' 
                    GROUP BY gt.id,gt.f_name 
                    ORDER BY gt.f_name;
                ";
                ...
                $sql="SELECT 
                    gt.id,
                    gt.f_name 
                    FROM `int_schools_x_sessions_log` AS s 
                    ...
                    <div class="tab-content">
                        ...
                        <div id="sectionB" class="tab-pane fade in active">
                            <form id="search-users" method="GET" action="" class="row">
                                ...
                                <div class="col-md-3">          
                                    &nbsp;<input name="action" 
                                        class="btn" value="Search" type="submit" style="width:100%; padding: 5px" >
                                </div>
                            </form>

                            <input id="myInput" type="text" 
                                placeholder="Please enter Paypal Email ID" 
                                class="form-control"
                                style="width: 100%">
                            <br>
                            ...
                            <div class="ct_heading clear">
                                ...
                                <ul>
                                    <li>				
                                        <button id="delete-user" style="color: #fff; background-color:#d9534f;padding: 10px 22px" type="submit" name="paybypaypal">Pay by Paypal</button>
                                    </li>
                                    ...
                                    <tbody id="myTable">
                                        ...
                                        ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <strong class="text-primary">Tutor:</strong><?=$tut_th['f_name']." ".$tut_th['lname']?>
                                                            <br/>
                                                            <strong class="text-primary">Session id:</strong><?=$row['id']?>
                                                            <br/>
                                        <?php 
                                        ...
                                        ?>
                                                            ...
                                                            if($row['add_observer'])
                                                            {
                                                                if($row['observer_confirm'] == 1)
                                                                    echo '<span class="btn btn-success btn-xs" style="float:right">Observer Claimed</span>';
                                                                else
                                                                   echo '<span class="btn btn-info btn-xs" style="float:right">Observer</span>';
                                                            }
                                                            else 
                                                                echo '<span class="btn btn-primary btn-xs" style="float:right">Tutor</span>';   
                                                    ?>
                                        <?php 
                                                    } else {
                                        ?>
                                        ...
                                        ?>
                                                            ...
                                                            <br/>
                                                            <a href="session-details.php?getid=<?=$row['id']?>" style="text-decoration:underline;">View</a> 
                                                        </td>                                                               
                                                    </tr>
                                        <?php
                                                }
