...
function myFunction() {
    ...
    //var r = confirm("Press a button!\nEither OK or Cancel.\nThe button you pressed OK,Reject session.");
     var info='If you cancel this job with less than 48 hours notice you risk be suspended and losing deducting the payments for 1 session.';
     var r = confirm(info);
    if (r == true) {
       // txt = "You pressed OK!";
        return true;
...
WHERE q.id =".$row['quiz_id']));
            ...
            ?>
                    ...
                    if($row['tut_teacher_id']==$tutor&&$in_sec>$ses_2hr_before){ 
                      // $tutor_id=$_SESSION['ses_teacher_id'];//'turor id'
                             //10 sec before 
                             $board_url=$row['braincert_board_url'];
                              ?> 
                             <br/>      
                             ...
                             <a href="<?=$board_url?>"  target="_blank"
                              ...
                              // $in_sec=-2701; //Testingn // $in_sec=-3; //Testingn
                                ...
                                ?> 
                           <button    type="submit" name="submit_feedback_edit" title="Edit Feedback"
                                      class="btn btn-info" value="<?=$row['id']?>">Edit Feedback</button> 
                       ...
                       <button style=" background-color:orange;
                                            color:#fff; border:1px solid orange" 
                                              type="submit" name="submit_feedback" title="Complete Session"
                                      class="btn btn-default" 
                                      value="<?=$row['id']?>"><?=$btn_text?></button>
                                    <?php  }?>         
