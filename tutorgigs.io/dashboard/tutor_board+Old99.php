...
####################################
if(isset($_GET['action'])&&$_GET['action']=='end_session'&&isset($_SESSION['live_ses_id'])){  /// p?action=end_session
  $curr_ses_id=$_SESSION['live_ses_id'];  
$query=mysql_query(" UPDATE int_schools_x_sessions_log SET tutor_mark_end=1 WHERE id=".$curr_ses_id);
// echo  $query; die;

header("Location:my-sessions.php"); exit;
}
...
///echo $iframe_url; die; 
 if(isset($_SESSION['live_ses_id'])&&$_SESSION['live_ses_id']>0){
  $curr_ses_id=$_SESSION['live_ses_id'];
   $ses_det=mysql_fetch_assoc(mysql_query(" SELECT * FROM int_schools_x_sessions_log WHERE 1 AND id=".$curr_ses_id));
  
 
  $ses_diff=strtotime($ses_det['ses_start_time'])-strtotime($cur_time);
  //echo $ses_diff;  //  ses_diff<0, then session_started
 }
...
<!-- <div class="logo">
	...
	<div class="container">
		<div class="row">
    <?php  if(isset($ses_diff)&&$ses_diff<0){?>
		<a href="tutor_board.php?action=end_session" onclick="return confirm('Are you sure?')">end this::<?php //=$ses_diff?></a>
    <?php   } //if(isset($ses_diff)&&$ses_diff<0){?>
