	...
	<div class="container">
		<div class="row">
			...
			<div id="content" class="col-md-8">
				...
				if($session_type=="dateBy")
				{
                               
					$endDate1 = date('Y-m-d', strtotime($endDate . ' +1 day'));
					$qq=" SELECT * FROM int_schools_x_sessions_log WHERE 1 ";
					$qq.=" AND feedback_id='0' ";
					$qq.=" AND tut_teacher_id='$id' ";
					$qq.=" AND ses_start_time between '".$startDate."' and '".$endDate1."'";

					if(isset($_GET['id']))
					echo $qq.=" AND id='".$_GET['id']."'";
...
<div class="col-md-12" style="padding: 10px">
						<div class="col-md-3">
						<select name="session_type" id="filterSelect" class="form-control spcs">
							<option value="all" <?php echo (isset($session_type)&&$session_type=="all")?'selected':NULL; ?> >All</option>
							<option value="dateBy" <?php echo (isset($session_type)&&$session_type=="dateBy")?'selected':NULL; ?>>Date By Sessions</option>
						</select>
					</div>
					<div class="col-md-3">
						<label class="dateFiled">To:
<input type="date" name="startDate" value="<?php echo $startDate?>"  class="dateFiled">
...
<input type="date" name="endDate" value="<?php echo $endDate;?>"
			...
			<div class="col-md-3">
			<input name="action" class="btn btn-success spcs" value="Filter" type="submit">  
		</div>
 </div>
</form>
				...
				<form onsubmit="return myFunction();" id="form-manager" class="content_wrap" action="" method="post">
					<div class="ct_heading clear">
						...
						<!--<input type="button" id="btnExport" value="Export CSV" class="btn btn-success" />-->
					...
					<div class="clear">
						...
						if($error != '') {
							echo '<p class="error">'.$error.'</p>';
                                                    ...
                                                    ?>
							<table class="table-manager-user col-md-12" id="tblSession">
								<colgroup>
                           ...
                           <strong class="text-primary">Teacher:</strong><?=$int_th['first_name']; ?><br/>
                            ...
                            <strong class="text-primary">Session id:</strong><?=$row['id']?><br/>         
                               ...
                               <?=date_format(date_create($row['created_date']), 'F d,Y');?>       
                                      ...
                                      class="btn btn-default" value="<?=$row['id']?>">Session Survey</button> </td>
...
<script src="../myadmin/table2excel.js" type="text/javascript"></script>
    <script type="text/javascript">
        $(function () {
            $("#btnExport").click(function () {
                $("#tblSession").table2excel({
                    filename: "session_survey.xls"
                });
            });
        });
    </script> 