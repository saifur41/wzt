					...
					<div class="wizard-v3-content">
						<div class="wizard-form">
							...
							<form class="form-register" action="" method="post">
									 ...
									 <!-- fggf -->
												 ...
												 // 	echo $url_file;
												  	...
												  	if(isset($_GET['key'])){
                                                                ...
                                                                $sql_single = mysql_fetch_assoc(mysql_query("select *from training_management where id='".$_GET['key']."' order by id asc"));
                                                                 $sql = mysql_query("select *from training_management order by id asc");
                                                                 
                                                                 $change_url=$sql_single['file'];
                                                   	 	$set_key=$_GET['key'];
                                                   	 }else{
                                                              $sql_single = mysql_fetch_assoc(mysql_query("select *from training_management order by id asc limit 1"));
                                                   	 	$change_url=$sql_single['file'];// Firs file
                                                   	 	$set_key=$sql_single['id'];
                                                                $sql = mysql_query("select *from training_management order by id asc");
                                ...
                                $url_file='https://docs.google.com/gview?url=https://tutorgig.englishpro.us/training_docs/'.$change_url.'&embedded=true';
...
$key = 0;
                                                   while ($row = mysql_fetch_assoc($sql)) {
                                                       $key++;
                                                   	# code...
                                                   	$activ_clss=($row['id']==$set_key)?'btn btn-success btn-xs':'btn btn-link btn-xs';
                                 ...
                                 <a class="<?=$activ_clss?>"
                                  href="training.php?key=<?=$row['id']?>#document_area"><?=$row['name']?></a>
