...
AND ses_start_time between '$getdate' AND '$end_date' AND `create_id` = $login_role";   # tut_status
     ...
     $det=mysql_fetch_assoc(mysql_query("SELECT * FROM `schools` WHERE SchoolId=".$row['school_id']));   
      ...
      $quiz= mysql_fetch_assoc(mysql_query("SELECT * FROM `int_quiz` WHERE id=".$row['quiz_id']));
         ...
         // Post reques to assign teacher
                    $slot_str.='<a title="'.$str_title.'" style="'.$style.'"  href="/mymanager/list-tutor-sessions.php?s_id='.$sesid.'&session_type=all&action=Search"   class="'.$str_class.'">'.$old_end.'</a><br/>';#1
                   //$slot_str.=$row['tut_status'];
